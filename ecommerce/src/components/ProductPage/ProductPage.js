...
export default class ProductPage extends Component {
  ...
  moveTopImageThumbnailsHandler(elem, distance) {
    let top = elem.scrollTop;
    const stopPosition =
      elem.scrollTop - distance < 0 ? 0 : elem.scrollTop - distance;
    function frame() {
      top -= 5;
      elem.scrollTop = top;
      if (top <= stopPosition) clearInterval(timeId);
    }
    let timeId = setInterval(frame, 10);
  }
  moveBottomImageThumbnailsHandler(elem, distance) {
    let top = elem.scrollTop;
    const stopPosition =
      elem.scrollTop + distance > elem.offsetHeight
        ? elem.offsetHeight
            ...
            <div className="image-gallery">
              <div className="image-gallery-navigation">
                <div
                  ...
                  onClick={() => {
                    this.moveTopImageThumbnailsHandler(
                      document.getElementById("scroll-thumbnails"),
                      100
                    );
                  }}
                >
                ...
                <div
                  ...
                  onClick={() => {
                    this.moveBottomImageThumbnailsHandler(
                      document.getElementById("scroll-thumbnails"),
                      100
                    );
                  }}
